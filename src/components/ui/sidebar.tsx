...
// Keyboard shortcut for toggling the sidebar. Uses "." so the full
// combo reads as Cmd/Ctrl + "." — matches the shortcut the
// interaction model documents for show/hide of the sidebar.
  ...
  return (
    ...
    >
      ...
      <div
        ...
        data-side={side}
        className={cn(
          "fixed top-12 bottom-0 z-10 hidden h-[calc(100vh-3rem)] w-(--sidebar-width) transition-[left,right,width] duration-200 ease-linear data-[side=left]:left-0 data-[side=left]:group-data-[collapsible=offcanvas]:left-[calc(var(--sidebar-width)*-1)] data-[side=right]:right-0 data-[side=right]:group-data-[collapsible=offcanvas]:right-[calc(var(--sidebar-width)*-1)] md:flex",
...
// NOTE: local patch (not stock shadcn). Every `data-[active=true]:*`
// selector in this file originally shipped as `data-active:*`. React 19
// renders `data-active={false}` as the literal attribute
// `data-active="false"`, and Tailwind v4 compiles `data-active:*` to
// `[data-active]` (attribute-presence), so *every* SidebarMenuButton
// ends up with the active styling. `[data-active=true]` restricts the
// match to truly active items. Keep this patch when re-running
// `shadcn add sidebar`.
